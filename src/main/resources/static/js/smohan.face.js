...
$(function() {
	$.fn.smohanfacebox = function(options) {
		...
		for (i = 0; i < 60; i++) { // 通过循环创建60个表情，可扩展
			faceimg += '<li><a href="javascript:void(0)"><img onclick="selectImg(this)" src="/images/zoo/face/' + (i + 1)
	...
	$.fn.extend({
		replaceface : function(faces) {
			for (var i = 0; i < 60; i++) {
				faces = faces.replace('&lt;emt&gt;' + (i + 1) + '&lt;/emt&gt;', '<img style="display: inline;" src="/images/zoo/face/' + (i + 1)
						+ '.gif">');
			}
			$(this).html(faces);
		}
...
function selectImg(obj) {
    ...
    // $("#"+options.textid).focus();
    // $("#"+options.textid).val($("#"+options.textid).val()+$(this).attr("face"));
    $(obj).parent().parent().parent().parent().parent().parent().prev().children(".submit_content").off().insertContent($(obj).attr("face"));
